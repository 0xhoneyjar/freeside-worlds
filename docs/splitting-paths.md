...
`freeside-world` is a monorepo today. The four packages (`creator`, `base`, `protocol`, `registry`) coexist in one repo because they're co-evolving. Each can extract to its own repo when load-bearing pressure earns the split. This doc names the triggers.

Per [`loa-org-naming-conventions`](https://github.com/0xHoneyJar/loa-hivemind/blob/main/wiki/concepts/loa-org-naming-conventions.md): extracted packages take the attachment-prefix convention (`freeside-world-protocol`, `freeside-world-creator`, etc.) unless promotion to a top-level multi-consumer schema (like `score-vault`) is justified.
...
**Extract target**: `0xHoneyJar/freeside-world-protocol`
...
**Migration mechanism**: `git filter-repo --path packages/protocol/ --path-rename packages/protocol/:`. Preserve history. Update `freeside-world/packages/registry/bin/validate.ts` to fetch from the extracted repo's raw URL or pin to a commit.
...
**Extract target**: `0xHoneyJar/freeside-world-creator`
...
**Migration mechanism**: `git filter-repo --path packages/creator/ --path-rename packages/creator/:`. After extraction, COSMOGRAPHER continues to validate Stage 9 against `freeside-world-protocol/world-manifest.schema.json` (which by this point is also extracted).
...
**Extract target**: `0xHoneyJar/freeside-world-base`
...
- Update `freeside-world/packages/creator/skills/creating-worlds/SKILL.md` Stage 3 to reference `gh repo create <slug>-world --template 0xHoneyJar/freeside-world-base`

**Keep here**: `packages/base/docs/{PHASES,GRADUATION,COMPONENTS}.md` stay in `freeside-world/docs/` (these document patterns, not the scaffold itself).
...
- CI in `freeside-world` pushes registry YAML changes to the DB on PR-merge
- `freeside-world/packages/registry/worlds/*.yaml` remains the source-of-truth; DB is a derived mirror
...
For `freeside-world/packages/protocol/`, the consumer set today is internal (registry generator + creator validator). Extraction earns its way when:
...
|---|---|---|---|
| protocol | ✅ likely first | 2+ external repos consume | `freeside-world-protocol` |
| creator | ⏳ later | 500+ LOC stage machine OR sibling creators emerge | `freeside-world-creator` |
| base | 🌱 medium | needs GitHub Template-repo treatment | `freeside-world-base` |
...
- **Top-level extraction without multi-consumer**: extracting `protocol` to a top-level `world-protocol` repo when the only consumer is `freeside-world` itself is a churn-cost without payoff.
